...
bool SKY::LoadTextures(const QString& folder){
    ...
    if(!LoadTexture( SKY_FRONT, folder+"/skyfront.jpg" )) return false;
    if(!LoadTexture( SKY_BACK,folder+"/skyback.jpg" )) return false;
    if(!LoadTexture( SKY_RIGHT,folder+"/skyright.jpg" )) return false;
    if(!LoadTexture( SKY_LEFT, folder+"/skyleft.jpg" )) return false;
    if(!LoadTexture( SKY_TOP, folder+"/skytop.jpg" )) return false;
    if(!LoadTexture( SKY_BOTTOM, folder+"/skybottom.jpg" )) return false;
...
bool SKY::LoadTexture( int side, const QString& filename )
{
  ...
  if (image.load(filename))
    ...
    {
      qWarning("Sky texture loading failed for side %d, filename: %s",side, filename.toStdString().c_str());
      return false;
    }
  return true;
}
